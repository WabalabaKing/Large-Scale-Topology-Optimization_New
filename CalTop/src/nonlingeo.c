  	...
  	{   
		...
		{  
         ...
         friction heating */
      	...
      	if((ncont!=0)&&(*mortar<=1)&&  ((*ithermal!=2)||(iit==-1)))
	  			...
	  			if(icutb==0)
				{
	      			if(*mortar==1)
					{
		  				...
		  				if(*nstate_!=0)
						{
		      				if(maxprevcontel!=0)
							{
			  					RENEW(xstateini,double,*nstate_*mi[0]*(ne0+maxprevcontel));
    		...
    		while(icntrl==0)
			{
	  				...
	  				if(icascade==2)
					{
	  				...
	  				if((ncont!=0)&&(*mortar<=1)&&(ismallsliding==0)&&
						/*           for node-to-face contact: freeze contact elements for
             			...
             			((iit<=8)||(*mortar==1))&&
						/*           	for purely thermal calculations: freeze contact elements during complete step */
             			((*ithermal!=2)||(iit==-1)))
						{
	      					...
	      					*ne=ne0;*nkon=nkon0;
                      		...
                      		clearini,&theta,xstateini,xstate,nstate_,&icutb, &ialeatoric,jobnamef,&alea);
	  				...
	  				else if(*isolver==4)
					{
						...
						else if((*ithermal==2)&&(uncoupled))
						{
		  					...
		  					sgi_main(&ad[neq[0]],&au[nzs[0]],&adb[neq[0]],&aub[nzs[0]],
			   				&sigma,&b[neq[0]],&icol[neq[0]],iruc,&n1,&n2,token);
	      				}
						else
						{
      			...
      			if((*mortar==1)&&(iit!=1)&&(*ne-ne0>0)&&(*nmethod!=4))
				{
      			...
      			calcresidual(nmethod,neq,b,fext,f,iexpl,nactdof,aux2,vold,
	 			vini,&dtime,accold,nk,adb,aub,jq,irow,nzl,alpha,fextini,fini,
	 			islavnode,nslavnode,mortar,ntie,f_cm,f_cs,mi,nzs,&nasym,&idamping,veold,adc,auc,cvini,cv);
      			...
      			if(*iexpl<=1)
				{
	  				...
	  				if(*ithermal>1)
					{
	  				...
	  				FORTRAN(writecvg,(istep,&iinc,&icutb,&iit,ne,&ne0,ram,qam,cam,uam,ithermal));

	  				checkconvergence(co,nk,kon,ipkon,lakon,ne,stn,nmethod, kode,filab,een,t1act,&time,epn,ielmat,matname,enern, 
	     			xstaten,nstate_,istep,&iinc,iperturb,ener,mi,output,
             		ithermal,qfn,&mode,&noddiam,trab,inotr,ntrans,orab,
	     			ielorien,norien,description,sti,&icutb,&iit,&dtime,qa,
	     			vold,qam,ram1,ram2,ram,cam,uam,&ntg,ttime,&icntrl,
