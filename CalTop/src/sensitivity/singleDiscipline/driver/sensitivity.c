...
void sensitivity(double *co, int *nk, ITG **konp, ITG **ipkonp, char **lakonp,
	...
	double *eleVol, int *eval_PNORM)    
    {
        ...
        *dgdxdy=NULL;
        ...
        /* determining the global values to be used as boundary conditions for a submodel */
        ...
        for(i=0;i<*nobject;i++)
        {
            if((strcmp1(&objectset[i*324],"DISPLACEMENT")==0)||(strcmp1(&objectset[i*324],"X-DISP")==0)||(strcmp1(&objectset[i*324],"Y-DISP")==0)||(strcmp1(&objectset[i*324],"Z-DISP")==0))
            {
	            idisplacement=1;
            }
        ...
        for(i=0;i<*nobject;i++)
        {
            if((strcmp1(&objectset[i*324+18],"LE")==0)||(strcmp1(&objectset[i*324+18],"GE")==0))
            {
	            nconstraint++;
            }
        }
        /* EIGENFREQUENCY as objective should not be used with any other objective in the same step */
        if(((idisplacement==1)||(imass==1)||(ishapeenergy==1))&&(ieigenfrequency==1))
        {
        ...
        if(icoordinate==1)
        {
            ...
            FORTRAN(findextsurface,(nodface,ipoface,ne,ipkon,lakon,kon,konfa,ipkonfa,nk,lakonfa,&nsurfs,&ifreemax));
            ...
            FORTRAN(extfacepernode,(iponoelfa,inoelfa,lakonfa,ipkonfa,konfa,&nsurfs,&inoelsize));
            ...
            FORTRAN(getdesiinfo,(set,istartset,iendset,ialset,nset,mi,nactdof,&ndesi,nodedesi,ntie,tieset,itmp,nmpc,nodempc,ipompc,nodedesiinv,iponoel,inoel,lakon,ipkon,kon,&iregion,ipoface,nodface,nk));  
      
            SFREE(itmp);
            RENEW(nodedesi,ITG,ndesi);

            /* storing the elements to which each design variable belongs in field ialdesi */
            NNEW(istartdesi,ITG,ndesi+1);
            NNEW(ialdesi,ITG,*nkon);

            FORTRAN(elemperdesi,(&ndesi,nodedesi,iponoel,inoel,istartdesi,ialdesi,lakon,ipkon,kon,nodedesiinv,&icoordinate,&iregion));
            ...
            FORTRAN(normalsonsurface_se,(ipkon,kon,lakon,extnor,co,nk,ipoface,nodface,nactdof,mi,nodedesiinv,&iregion)); 
            ...
            */
            ...
            NNEW(inum,ITG,*nk);
            FORTRAN(createinum,(ipkon,inum,kon,lakon,nk,ne,&cflag[0],nelemload,nload,nodeboun,nboun,ndirboun,ithermal,co,vold,mi,ielmat,ielprop,prop));
        ...
        FORTRAN(desiperelem,(&ndesi,istartdesi,ialdesi,ipoeldi,ieldi,ne, istartelem,ialelem));
        ...
        if(*nbody>0)
        {
            ...
            for(k=1;k<=*nbody;k++)
            {
                FORTRAN(bodyforce,(cbody,ibody,ipobody,nbody,set,istartset, iendset,ialset,&inewton,nset,&ifreebody,&k));
        ...
        NNEW(nodorig,ITG,*nk);
        FORTRAN(gennactdofinv,(nactdof,nactdofinv,nk,mi,nodorig,ipkon,lakon,kon,ne));
        SFREE(nodorig);
