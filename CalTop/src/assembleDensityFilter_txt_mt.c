...
/**
 * @brief Streams a symmetric density-filter matrix block for one thread.
 *
 * Processes the element range [ne_start, ne_end) assigned to this thread.
 * For each receiver element i, it scans all donor elements j ∈ [0, ne0)
 * and, if dist(i,j) ≤ rmin_local and i ≠ j, computes a linear weight
 *
 *      w_ij = rmin_local - ||x_i - x_j|| ,
 *
 * then emits the symmetric triplets (i,j) and (j,i) to thread-local files
 * in 1-based indexing. It also tallies per-row sums consistent with the
 * emitted symmetric entries and writes them to a per-thread dsum file.
 *
 * Files produced (thread-local):
 *   - drow_<tid>.dat : row indices (1-based) for each triplet
 *   - dcol_<tid>.dat : col indices (1-based) for each triplet
 *   - dval_<tid>.dat : weights w_ij for each triplet
 *   - dnnz_<tid>.dat : neighbor count for each i in this thread’s block
 *   - dsum_<tid>.dat : partial row sums (length = ne0), to be reduced later
 *
 * Progress is printed to stderr as an in-place text progress bar. On I/O or
 * allocation errors, the function reports to stderr and terminates the process.
 *
 * Important behavior/assumptions:
 *   • Diagonal entries (i == j) are skipped.
 *   • For each unordered pair {i,j}, this thread emits both (i,j) and (j,i)
 *     when visiting receiver i. When the outer loop later visits receiver j,
 *     the same two entries will be emitted again by that thread/range, so
 *     downstream assembly must either sum duplicate triplets or you must
 *     later coalesce them. Row-sum accumulation mirrors this duplication.
 *   • Weights use a linear kernel; there is no exponent q (effectively q = 1).
 *   • Indices written to files are 1-based to match CalculiX/ABAQUS-style IO.
 *
 * @param args_ptr  Opaque pointer to a ThreadArgs struct with:
 *                  - int    thread_id      : unique thread id (for filenames)
 *                  - int    ne0            : total number of elements
 *                  - int    ne_start       : first receiver element (inclusive)
 *                  - int    ne_end         : last  receiver element (exclusive)
 *                  - double *elCentroid    : flat array of size 3*ne0 (xi,yi,zi)
 *                  - double rmin_local     : filter radius r_min (> 0)
 *                  - int   *fnnzassumed    : per-row nnz guard (max neighbors)
 *                  - int   *filternnzElems : output array storing nnz per row i
 *
 * @return void*    Always returns NULL (pthreads signature).
 *
 * @sideeffects
 *   Creates/writes the five files listed above; writes progress to stderr;
 *   may call exit(EXIT_FAILURE) on error.
 *
 * @pre elCentroid points to 3*ne0 valid coordinates; rmin_local > 0.
 * @note If any row could have zero neighbors when skipping the diagonal,
 *       ensure the normalization step guards against zero row-sums.
 */
