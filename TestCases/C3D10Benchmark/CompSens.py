...
def read_adjoint_stress(path="compliance_sens.csv"):
...
def read_pnorm_from_objectives(path="objectives.csv"):
    ...
    try:
        j = header.index("COMPLIANCE")
    except ValueError:
        raise ValueError("COMPLIANCE column not found in objectives.csv header")
    return float(vals[j])
...
def verify_stress_sens_cfd(job, p, r, f, pexp, sigmin, sigrelax,
    ...
    run_caltop(job, p, r, f, pexp, sigmin, sigrelax)
    adjoint = read_adjoint_stress("compliance_sens.csv")
    ...
    for i in idxs:
        ...
        if h_minus > 0.0:
            #h_plus > 0.0 and and abs(h_plus - h_minus) < 1e-15:
            ...
            cfd = (pnorm0- p_minus) / ( h)
            #print("Current Density : ",dens_minus)
        ...
        CFD.append(cfd)
        denom = max(eps, abs(cfd))
        rel_err = ((cfd - adjoint[i]) / denom) 
        if cfd!=0:
            ratio = adjoint[i]/cfd
        else:
            ratio =999999
...
def main():
    ...
    ap.add_argument("-i", "--input", default="C3D10", help="Job/input name without extension (default: SCB)")
    ap.add_argument("-p", "--penal", type=float, default=5.0)
    ap.add_argument("-r", "--radius", type=float, default=0.2)
    ap.add_argument("-f", "--filternnz", type=int, default=10)
    ...
    ap.add_argument("--max-elems", type=int, default=1, help="Limit number of elements")
