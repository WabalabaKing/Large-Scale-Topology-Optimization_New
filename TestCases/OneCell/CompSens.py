...
def verify_stress_sens_cfd(job, p, r, f, pexp, sigmin, sigrelax,
    ...
    for i in idxs:
        ...
        if h_plus > 0.0 and h_minus > 0.0 and abs(h_plus - h_minus) < 1e-15:
            ...
            cfd = (p_plus - p_minus) / (2.0*h)

        elif h_plus > 0.0 and h_minus > 0.0:
            # Asymmetric central difference (clipped differently)
            dens_plus = density[:]
            dens_plus[i] = rho + h_plus
            write_density(dens_plus, "density.dat")
            run_caltop(job, p, r, f, pexp, sigmin, sigrelax)
            ...
            cfd = (p_plus - p_minus) / (h_plus + h_minus)
...
def main():
    ...
    ap.add_argument("-i", "--input", default="TwoCells", help="Job/input name without extension (default: SCB)")
    ap.add_argument("-p", "--penal", type=float, default=2.0)
    ap.add_argument("-r", "--radius", type=float, default=0.1)
    ap.add_argument("-f", "--filternnz", type=int, default=500)
    ap.add_argument("--pexp", type=int, default=2.0)
    ap.add_argument("--sigmin", type=float, default=0.001)
    ap.add_argument("--sigrelax", type=float, default=0.001)
    ap.add_argument("--h", type=float, default=1e-5, help="Constant absolute perturbation (default 1e-6)")
