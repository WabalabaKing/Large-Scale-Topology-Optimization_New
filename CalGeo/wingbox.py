...
"""
===============================================================================
USAGE NOTE – SU2 to CalculiX Mesh Preprocessor
===============================================================================

This script converts an SU2 tetrahedral mesh into CalculiX .nam format and
extracts boundary node sets and skin element layers for CalTop.

Basic Usage
-----------
    python script.py mesh.su2 <SkinMarkerList> [options]

Required Arguments
------------------
mesh.su2
    Path to the input SU2 mesh file.

SkinMarkerList
    One or more SU2 MARKER_TAG names defining the skin region.
    Each entry must match the marker name exactly (case-sensitive).
    ...
    Examples:
        ...
        skin:2
        wing_surface:3
        skin:2 wing_surface:1

    If ":layers" is omitted, 1 layer is used by default.

What the Script Does
--------------------
1. Converts SU2 nodes and tetrahedral elements to CalculiX format:
       -> mesh.nam

2. Extracts node sets:
       MARKER_TAG= fixed     -> Nfix1.nam
       MARKER_TAG= surface   -> NSurface.nam

3. Builds a skin element list:
       - Finds elements touching the specified skin marker(s)
       - Expands outward by the specified number of adjacency layers
       - Writes 1-based element IDs to:
             skinElementList.nam

Notes
-----
- Marker names must exactly match "MARKER_TAG= <name>" in the SU2 file.
- Element IDs in skinElementList.nam are written 1-based for CalculiX.
- If no valid skin markers are found, skinElementList.nam is not written.
- Supports tetrahedral elements (C3D4).

Example
-------
    python script.py mesh.su2 skin:2

This selects all elements touching MARKER_TAG= skin and grows 2 layers.
